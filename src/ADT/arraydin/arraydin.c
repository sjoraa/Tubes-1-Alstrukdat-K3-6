...
boolean stringsama(char *str1, char *str2){
    int i;
    if (panjangstring(str1)!=panjangstring(str2)){
        return false;
    }
    else{
        for (i=0;i<panjangstring(str1);i++){
            if (str1[i]!=str2[i]){
                return false;
            }
        }
...
IdxType SearchArrayDin(ArrayDin array, ElType el)
{
    ...
    for (i = 0; i < array.Neff; i++)
    {
        if (stringsama(array.A[i], el) == true)
        {
            return i;
        }
    }
